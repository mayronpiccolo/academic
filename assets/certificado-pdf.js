  ...
  function generate(data, opts) {
    ...
    return loadFonts(base).then(function (fonts) {
      ...
      var issueRuns = [
        ...
        [{ t: "mayronpiccolo.com/psicologia-no-sofa.html", f: "Archivo", c: MUTED }]
      ];
      issueRuns.forEach(function (r, i) { drawRuns(doc, r, 7.9, rcx, ry + 7 + i * 4.6, colW, 4.6, MUTED); });

      doc.setProperties({ title: "Certificado — " + data.name, subject: data.title, author: "Psicologia no Sofá", creator: "mayronpiccolo.com" });
      return doc;
    });
  }
